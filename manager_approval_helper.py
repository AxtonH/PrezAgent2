...
from activity_tracker import track_manager_approval, track_manager_overtime_approval
...
def handle_manager_overtime_approval(query, employee_data):
    ...
    if intent == 'view_pending_overtime':
        ...
        if result.get('success'):
            # Track only approvals as activities
            if intent == 'approve_overtime':
                try:
                    req = st.session_state.odoo_models.execute_kw(
                        st.session_state.db,
                        ...
                        'read',
                        [[request_id]],
                        {'fields': ['request_owner_id', 'name']}
                    )
                    employee_name = req[0]['request_owner_id'][1] if req and req[0].get('request_owner_id') else 'Unknown'
                except Exception:
                    employee_name = 'Unknown'
                track_manager_overtime_approval(employee_name=employee_name, request_id=request_id)
